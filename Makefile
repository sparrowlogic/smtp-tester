...
MVN := ./mvnw -B
DOCKER_IMAGE ?= ghcr.io/sparrowlogic/smtp-tester
DOCKER_TAG ?= dev
...
#
# `clean` first, and not negotiably. Maven copies resources into target/ but never removes one that
# has been deleted from src/, so a stale file keeps being packaged and tested against long after it
# is gone from the source. That is not hypothetical: deleting
# META-INF/spring/...AutoConfiguration.imports left the old copy in target/classes naming classes
# that no longer existed, and the gate passed green for several runs before every Spring context in
# the suite failed at once. A gate that can pass on output the source no longer describes is not a
# gate. The suite takes seconds; a false green costs far more than the rebuild.
