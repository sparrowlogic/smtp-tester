...
smtp-tester is a published, MIT-licensed library **and** a public container image. Both are
consumed by people who cannot see this conversation, so the bar is what a stranger can safely
depend on, not what happens to work on this machine.
...
It ships in three shapes from one codebase, which is the constraint most decisions come back to:

1. **A container** — `smtp-tester-<version>-exec.jar` run by the Dockerfile.
2. **A library** — the plain jar, added as a `<dependency>`, auto-configuring itself inside a host
   application's tests.
3. **An MCP server** — streamable HTTP at `/mcp`, on the same port as the inbox.
...
| `config` | properties and auto-configuration |

---

## House rules

### Library mode is a real constraint

The jar is embedded in other people's applications. Therefore:

- **No component scanning.** Every bean is declared explicitly in an `@AutoConfiguration` class, so
  the bean graph is identical standalone and embedded. Do not add `@Component` / `@Service` /
  `@RestController`-by-scanning.
- **Every bean is `@ConditionalOnMissingBean`** and named `smtpTester*`, so a host can replace one
  and nothing collides.
- **No `application.yaml` in `src/main/resources`.** It would override a host's own `server.port`.
  Standalone-only defaults belong in `SmtpTesterApplication`.
- **Nothing global.** A servlet filter is scoped to `smtptester.web.base-path`, not `/*`.

`LibraryModeTest` is what catches a regression here. Keep it passing.
...
- **Never log credentials or full bodies by default.** `smtptester.logging.include-body` exists so
