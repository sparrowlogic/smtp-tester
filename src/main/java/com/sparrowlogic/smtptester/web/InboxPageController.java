 ...
 * registers engine and resolver beans that would collide with a host application's own JTE setup
 * when this jar is embedded, and going through {@code createPrecompiled} guarantees the packaged
 * artifact renders from classes compiled into the jar rather than from {@code src/main/jte} on
 * disk — which exists on a developer machine and does not exist inside the container.
 */
@Controller
@RequestMapping("${smtptester.web.base-path:}")
