...
- **Component scanning.** Every bean is declared in `SmtpTesterConfiguration` or
  `SmtpTesterWebConfiguration`, both named on `@Import` in `SmtpTesterApplication`, and named
  `smtpTester*`. The bean graph should be readable from those two files alone.
- **Reintroducing library mode.** This ships as a container only — no `@AutoConfiguration`, no
  `AutoConfiguration.imports`, no Maven Central publishing, and no "also works as a dependency"
  in the README.
...
smtp-tester version, how you are running it (container or executable jar), and what you expected.
